...
import { ArrowRight, Mail, Snowflake, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle, PartyPopper, Loader } from "lucide-react";
...
const loginSteps = [
    ...
    { message: "Fetching profile...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
const DefaultLogo = () => (
  <motion.div 
    ...
    className="bg-sky-500 text-white rounded-xl p-2 shadow-lg shadow-sky-500/20"
  > 
...
// --- MAIN COMPONENT ---
export const AuthPage = ({ logo = <DefaultLogo />, brandName = "Indian Refrigeration" }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [mode, setMode] = useState(location.state?.mode || "signup"); // "login" | "signup"
  ...
  const handleFinalSubmit = (e) => {
    ...
    const steps = mode === 'signup' ? modalSteps : loginSteps;
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center w-full">
                            <GlassButton contentClassName="flex items-center justify-center gap-2" size="sm" className="w-full max-w-[200px]"><GoogleIcon /><span className="font-semibold text-foreground">Continue with Google</span></GlassButton>
                        </div></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{mode === 'signup' ? 'Your password must be at least 6 characters long.' : 'Welcome back! Please enter your password.'}</p></BlurFade>
                    </motion.div>}
                                ...
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        ...
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            ...
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type={mode === 'signup' ? "button" : "submit"} onClick={mode === 'signup' ? handleProgressStep : undefined} size="icon" aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground">{mode === 'signup' ? <ArrowRight className="w-5 h-5" /> : <Loader className="w-5 h-5 animate-spin" />}</GlassButton></div>
                                        </div></div>
                                    </div>
                ...
                <BlurFade delay={0.5}>
                    <p className="text-sm text-muted-foreground">
                        ...
                        <button onClick={toggleMode} className="text-foreground font-semibold hover:underline underline-offset-4 transition-all">
